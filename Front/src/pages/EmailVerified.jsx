...
export default function EmailVerified() {
    ...
    const verifyEmail = async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/verify/email/confirm?token=${token}`
        );
        if (response.ok) {
  ...
  return (
    <div className="flex flex-col justify-baseline items-center h-screen py-[200px]">
      {isResponseOk ? (
        <>
          <svg
            width="100px"
          ...
          <span className="text-2xl font-medium mt-6">Email Verified</span>
        </>
      ) : (
        <>
          <svg
